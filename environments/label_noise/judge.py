...
class LabelNoiseJudge(JudgeInterface):
    ...
    def _load_model(self, model_path: str, device: str = "cpu") -> Tuple[PolicyValueNet, Dict[str, Any]]:
    ...
    def _gradient_norm(self, model: PolicyValueNet, features: np.ndarray, labels: np.ndarray, device: str = "cpu") -> float:
        model.zero_grad(set_to_none=True)
        x = torch.from_numpy(features[:128]).to(device)
        y = torch.from_numpy(labels[:128]).to(device)
        logits, _ = model(x)
    ...
    def evaluate(self, model_path: str, device: str = "cpu") -> JudgeResult:
        ...
        try:
            model, metadata = self._load_model(model_path, device)
        ...
        train_metrics = self._evaluate_split(model, train.features, train.labels, device)
        val_metrics = self._evaluate_split(model, val.features, val.labels, device)

        gap = float(train_metrics["accuracy"] - val_metrics["accuracy"])
        grad_norm = self._gradient_norm(model, train.features, train.labels, device)
